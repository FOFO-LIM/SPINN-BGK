...
class x3v3(base_v3):
    ...
    def loss(self, params, domain):
        ...
        loss_r, loss_ic, *loss_bc = jax.tree.map(
    ...
    def moments_neq(self, params, t, x, y, z):
        ...
        int_f = jax.tree.map(lambda fv: jnp.dot(self.w, fv), tensor_v)
        int_vf = jax.tree.map(lambda fv: jnp.dot(self.wv, fv), tensor_v)
        int_v2f = jax.tree.map(lambda fv: jnp.dot(self.wvv, fv), tensor_v)
        # moments
